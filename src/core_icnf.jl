...
function callback_f(
    ...
    loss::Function,
    data::DataLoader{T3},
)::Function where {T<:AbstractFloat,AT<:AbstractArray,T2<:AbstractMatrix,T3<:Tuple{T2}}
    xs, = first(data)
    function f()::Nothing
        vl = loss(icnf, xs)
...
Distributions._logpdf(d::ICNFDist, x::AbstractVector) =
    first(Distributions._logpdf(d, hcat(x)))
Distributions._logpdf(d::ICNFDist, A::AbstractMatrix) = inference(d.m, TestMode(), A)
Distributions._rand!(rng::AbstractRNG, d::ICNFDist, x::AbstractVector) =
    (x .= Distributions._rand!(rng, d, hcat(x)))
Distributions._rand!(rng::AbstractRNG, d::ICNFDist, A::AbstractMatrix) =
