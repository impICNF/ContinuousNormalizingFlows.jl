...
function callback_f(
    ...
    loss::Function,
    data::DataLoader{T3},
)::Function where {T<:AbstractFloat,AT<:AbstractArray,T2<:AbstractMatrix,T3<:Tuple{T2,T2}}
    xs, ys = first(data)
    function f()::Nothing
        vl = loss(icnf, xs, ys)
...
Distributions._logpdf(d::CondICNFDist, x::AbstractVector) =
    first(Distributions._logpdf(d, hcat(x)))
Distributions._logpdf(d::CondICNFDist, A::AbstractMatrix) =
    inference(d.m, TestMode(), A, d.ys[:, 1:size(A, 2)])
Distributions._rand!(rng::AbstractRNG, d::CondICNFDist, x::AbstractVector) =
    (x .= Distributions._rand!(rng, d, hcat(x)))
Distributions._rand!(rng::AbstractRNG, d::CondICNFDist, A::AbstractMatrix) =
